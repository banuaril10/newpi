				...
				<button class="btn btn-danger" type="button" id="btn-cetak-tinta"><div class="item1"><b>CETAK HARGA PROMO</b></div></button>
					...
					<table class="table table-bordered" id="example">
						...
						foreach ($connec->query($sql_list) as $row) {
							$harga_last = 0;
							$cek_disc = "select todate, afterdiscount from pos_discount where (fromdate <= '".date('Y-m-d')."' and todate >= '".date('Y-m-d')."') and sku = '".$row['sku']."'";
...
function formatRupiah(angka, prefix){
			...
			document.getElementById("btn-cetak-tinta").addEventListener("click", function() { //ini yg dipake
				...
				for (let i = 0; i < array.length; i++) {
						...
						if(panjangharga < 1000){
							
							gope = " &nbsp &nbsp &nbsp &nbsp ";
						}
