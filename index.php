...
$cmd_alter_salesheader = ['ALTER TABLE pos_dsales ADD COLUMN IF NOT EXISTS status_sales_header numeric DEFAULT 0 NOT NULL;'];
$cmd_alter_salesline_old = ['ALTER TABLE pos_dsales ADD COLUMN IF NOT EXISTS status_sales_line numeric DEFAULT 0 NOT NULL;'];
foreach ($cmd_alter_salesline as $r){
	$connec->exec($r);
}

foreach ($cmd_alter_salesheader as $r){
	$connec->exec($r);
}

foreach ($cmd_alter_salesline_old as $r){
	$connec->exec($r);
}
	...
	$.ajax({
		...
		success: function(dataResult){
			...
			if(dataResults.result=='1'){
				...
				$('#notif1').html(msg+" <a target=_blank href='https://idolmart.co.id/live/pi/doc_pi.php'>Link update</a> <button class='btn btn-danger' type='button' onclick='updateVersion();'>Update</button>");
