		...
		<div class="card">
			...
			<div class="tables">
				...
				<div class="table-responsive bs-example widget-shadow">				
					...
					<button onclick="cetakGeneric('<?php echo $_GET['m_pi']; ?>', '<?php echo $rack_name; ?>','<?php echo $dn; ?>');" class="btn btn-danger">Cetak Selisih Generic</button>	
					<button onclick="cetakGenericMike('<?php echo $_GET['m_pi']; ?>', '<?php echo $rack_name; ?>','<?php echo $dn; ?>');" class="btn btn-danger">Cetak Selisih Generic Mike</button>	
...
function cetakGenericMike(mpi, rn, dn){
...
function cetakPdfStruk(mpi, rn, dn){
				...
				for(let i = 0; i < dataResult.length; i++) {
						...
						var barcode = data.barcode;
							...
							if(number == panjang){
								...
								var mywindow = window.open('', 'my div', 'height=600,width=800');
							/*optional stylesheet*/ //mywindow.document.write('<link rel="stylesheet" href="main.css" type="text/css" />');
							mywindow.document.write('<style>*{font-family: Verdana; margin-left:15px; font-size: 8px; } table, th, td {border: 1px solid black;border-collapse: collapse;font-family: Verdana}@media print{@page {size: potrait; width: 58mm; font-family: Verdana; font-size: 8px; margin:0px;}}table { page-break-inside:auto }tr{ page-break-inside:avoid; page-break-after:auto }</style>');
							mywindow.document.write(html);
