			...
			<div class="tables">			
				...
				<p id="notif1" style="color: red; font-weight: bold"></p>				
					<table class="table table-bordered" id="example">
						...
						<?php 
						$sql_list = "select COUNT(sku) ,sku, name from inv_mproduct group by sku, name HAVING COUNT(sku) > 1";
						$no = 1;
						foreach ($connec->query($sql_list) as $row) {
							$sql_rack = "select rack_name from inv_mproduct where sku = '".$row['sku']."'";
							$rackk = "";
							foreach ($connec->query($sql_rack) as $rr) {
								$elements[] = $rr['rack_name'];
								
								
							}
							$rackk = implode(', ', $elements);
						...
						?>
							...
							<tr>
								...
								<td><?php echo $row['sku']; ?> <br><b>(<?php echo $rackk; ?>)</b></td>
