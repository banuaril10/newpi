        ...
        if(!empty($query))
        {
			...
			foreach($query as $r){
				...
				if($r['status'] == 1){
					...
					if($_SESSION['name'] == 'Ka. Toko' || $_SESSION['name'] == 'Wk. Ka Toko'){
						$st = "<font style='background-color: green; color: #fff; padding: 5px'>Completed</font> &nbsp <button type='button' class='btn btn-warning' onclick='cetakStrukDetail(\"".$r['cashinid']."\")'>Reprint </button>";
