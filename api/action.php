		...
		if(isset($_SESSION['username']) && !empty($_SESSION['username'])) {
			...
			if($statement){
				...
				foreach($j_hasil as $r) {
					...
					if($ketemu == 1){
						$statement1 = $connec->query("insert into m_piline (m_pi_key, ad_org_id, isactived, insertdate, insertby, postdate, m_storage_id, m_product_id, sku, qtyerp, qtycount, qtysales, price, status, qtysalesout, status1, barcode) 
...
VALUES('".$item['ad_client_id']."', '".$item['ad_org_id']."', '1', '".date('Y-m-d H:i:s')."', '".$item['createdby']."', '".$item['updatedby']."', '".date('Y-m-d H:i:s')."', '".$item['m_product_category_id']."', '".$item['value']."', '".$item['name']."', '".$item['description']."', '".$item['issummary']."');");
		...
		foreach ($connec->query($getinv) as $gi) {
			...
			$cekqty = "select qtycount from m_piline where sku = '".$gi['sku']."' and date(insertdate) = date(now())";
			$result = $connec->query($cekqty);
			$count = $result->rowCount();
			...
			if($count > 0){
				...
				foreach ($result as $tot) {
					...
					$jumqty = (int)$qtycount + (int)$gi['jumqty'];
							...
							$upcount = $connec->query("update m_piline set qtycount='".$jumqty."' where sku='".$gi['sku']."' and date(insertdate)=date(now()) ");
							if($upcount){
