...
if($_GET['modul'] == 'inventory'){	
	...
	if($_GET['act'] == 'input'){
		...
		if(isset($_SESSION['username']) && !empty($_SESSION['username'])) {
				...
				$cekrak = "select count(m_pi_key) jum from m_pi where rack_name='".$rack."' and status not in ('3','4','5') and date(insertdate) = date(now())";
...
// echo $sql;
		...
		if($r['value'] != ''){
			...
			$sql = "update pos_mproduct set barcode='".$r['value']."', shortcut = '".$r['upc']."', name = '".$r['name']."' where sku='".$r['sku']."'";
			// echo $sql;
			$upcount = $connec->query($sql);
